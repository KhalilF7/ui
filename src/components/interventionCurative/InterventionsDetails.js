...
export default function InterventionsDetails() {
  ...
  const getTechnice = mat => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/technicien/${mat}`)
      .then(resposne => {
        setTech(resposne.data);
      });
  };
  const getDate = date => {
  ...
  return (
    <>
      {loading && <Spinning />}
      {!loading && intervention && machine && technicien && (
        <div
          style={{
            margin: "100px",
            display: "flex",
                          ...
                          <>
                            <Grid
                              ...
                              <Grid item>
                                {intervention.technicine !== null &&
                                  tech === undefined &&
                                  getTechnice(intervention.technicine)}
                                {tech && (
                                  <>
                                    <div>
                                      <h5> technicine : </h5> {tech.nom}{" "}
                                      {tech.prenom}
                                    </div>
                                  </>
                                )}
